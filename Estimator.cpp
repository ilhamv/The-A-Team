...
// Pulse Height Tally Estimator
////////////////////////////////////

// Score at events
void PulseHeight_Estimator::score( const Particle_t& P, const double told, const double track /*= 0.0*/ )
{
    x = 0.0;
    
    if(P.region() == regPtr){         //pointer to the region of interest
        x =    P.energy()*P.weight(); //particle enters the region
    }
    else{
        x =  - P.energy()*P.weight(); //particle exits the region
    }
    tally_hist += x;
}
...
// Closeout history
// Update the sum and sum of squared, and restart history sum of all tallies
void PulseHeight_Estimator::endHistory()
{
    nhist++;
    
    //FOR ENERGY BIN. Find the energy in the bin grid and score 1.0 (not weight)
    int k = Binary_Search(tally_hist, grid);
    if(k >= 0 && k < grid.size() ){
        
        tallySumVec[k]     += 1.0;
        tallySquaredVec[k] += 1.0; //1.0*1.0;
        
        //FOR TOTAL TALLY
        tally_sum     += 1.0;
        tally_squared += 1.0; //1.0*1.0;
    }
...
void PulseHeight_Estimator::report( std::ostringstream& output, const double trackTime )
{
    ...
    double var  = std::sqrt( ( tally_squared / nhist - mean*mean ) / nhist ) / mean;
    
    //output the results
    output << "Estimator report: " << e_name << " tally for " <<  regPtr->name() << " region \n";
    output <<"=========================================================== \n";
    //FOR ENERGY BIN
    ...
    //FOR TOTAL
    output << "\nTotal pulse height tally:           " <<std::setw(18) << std::left  <<mean<<std::setw(18) << std::left  <<var<<"\n\n";
}
